...
import { Settings, User as UserIcon, Smartphone, Printer, Crown, Loader2, Save, Palette, Moon, Sun, Check, Camera, Trash2 } from "lucide-react";
...
function ConfigPage() {
  ...
  const [profile, setProfile] = useState<ProfileData>({
    full_name: "", phone: "", store_name: "", cnpj: "", avatar_url: "",
        ...
        <TabsContent value="perfil" className="space-y-4 mt-4">
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            <AvatarUploader
              ...
              fallback={(profile.full_name || user?.email || "U").charAt(0).toUpperCase()}
...
function AvatarUploader({
  ...
  async function remove() {
    await supabase.from("profiles").update({ avatar_url: null } as any).eq("id", userId);
    onChange("");
    ...
    <div className="flex items-center gap-4 pb-2 border-b border-border">
      ...
      <div className="flex-1 space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Quadrada, mín. <strong>256×256px</strong> (recomendado 512×512). PNG ou JPG, até 2MB.
        </p>
        <div className="flex gap-2">
          ...
          {avatarUrl && (
            <Button type="button" size="sm" variant="ghost" onClick={remove} className="text-destructive">
