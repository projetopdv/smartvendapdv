...
import { ShieldCheck, Plus, Loader2, Trash2, ShieldAlert, Crown, Sparkles } from "lucide-react";
...
function UsersPage() {
  ...
  async function load() {
    setLoading(true);
    const [{ data: profiles }, { data: roles }, { data: subs }, { data: pls }] = await Promise.all([
                    ...
                    <TableCell>
                      {u.plan_name ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium">
                          <Crown className="h-3 w-3" /> {u.plan_name}
                        </span>
                      ) : (
